...
#Bastien PAVION# et #Marion THUAUD#
...
Ce code semble structuré en couches très basiques, ce qui ne correspond pas à une architecture en couches à proprement parler.


*Où se trouve le code métier (voir la règle de gestion RG 1) ?*

Le code métier se trouve dans le controller de la ToDoList.

*Cette règle est-elle facilement testable par un test unitaire ?*

Cette règle peut être testable mais dans le contexte actuel ce n'est pas super simple. Spring rend la tâche plus complexe que du Java natif. Les tests devront être crées à l'extérieur du controller pour la maintenabilité du code.
...
Ici la commande permet de lancer une nouvelle instance de conteneur, en l'occurence un conteneur Docker. La mention du "-it" permet de lancer ce conteneur en mode interactif. Le "-e" permet de renseigner une variable d'environnement, dans ce cas le mot de passe Postgresql. Le "-p" permet de rensigner le numéro de port de lancement.

* Compléter le code manquant dans la méthode `TodoListController.createTodoItem()`

*Pourquoi `todoItemRepository` est-il `null` ? Quelle est la meilleure façon de l'injecter ?*
`todoItemRepository` est `null` car il n'a pas été injecté. La meilleure méthode pour l'injecter et de rajouter la mention @Inject au dessus du constructeur.
...
L'architecture hexagonale, également appelée "Ports and adapters", est un modèle de conception qui se base sur le principe de l'isolation. Chaque aspect spécifique de l'application est séparé dans une couche claire qui n'a pas de responsabilité transverse. Les interactions avec l'extérieur sont abstraites sous forme de ports et les implémentations concrètes sont des adapteurs. Cette architecture facilite le test unitaire par son indépendance. Les dépendances sont inversés de telle sorte que les composants métier ne dépendent pas des détails de l'implémentation mais des abstractions.
...
Les tests les plus pertinents pour les adapteurs sont les test d'intégration. Un test d'intégration contrairement au test unitaire va s'assurer de la communication entre les différentes parties du système. Les tests unitaires eux vont se focaliser sur une méthode précise.
